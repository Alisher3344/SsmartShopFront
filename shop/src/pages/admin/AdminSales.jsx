...
export default function AdminSales() {
  ...
  const { products, stores, saleProducts, toggleSale } = useAdminData();
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Mahsulotlarni aksiyaga qo'shing — bosh sahifaning "Chegirmalar" bo'limida ko'rsatiladi
        </p>
      </div>
      ...
      <div>
        <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wider mb-3 flex items-center gap-2">
          <FluentEmoji name="fire" size={16} />
          Faol aksiyalar ({saleProducts.length})
        </h2>

        {saleProducts.length === 0 ? (
          <div className="card p-8 text-center text-gray-500">
            <Tag className="w-10 h-10 mx-auto mb-2 text-gray-300" />
            <p className="text-sm">Hozircha aksiyada mahsulot yo'q</p>
            <p className="text-xs text-gray-400 mt-0.5">
              Pastdagi ro'yxatdan tanlab "Aksiyaga qo'shish" tugmasini bosing
            </p>
          </div>
        ) : (
              ...
              return (
                <div key={p.id} className="card overflow-hidden flex flex-col group relative">
                  ...
                  <div className="aspect-square bg-gray-50 relative">
                    ...
                    <div className="absolute bottom-2 left-2 bg-orange-500 text-white text-[10px] font-bold px-2 py-1 rounded-md shadow-md flex items-center gap-1">
                      <FluentEmoji name="fire" size={11} /> AKSIYA
                    </div>
                  </div>
                  <div className="p-3 flex flex-col flex-1">
                    ...
                    >
                      ...
                      {busyId === p.id ? 'Olinmoqda...' : 'Aksiyadan olish'}
                    </button>
                  </div>
                </div>
              );
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-white/5 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="text-right px-4 py-3 font-medium text-gray-600 hidden sm:table-cell">Narx</th>
                    <th className="text-right px-4 py-3 font-medium text-gray-600 hidden lg:table-cell">Eski narx</th>
                    ...
                    <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-white/5">
                      ...
                      <td className="px-4 py-3 text-center">
                        <button
                          onClick={() => handleToggle(p.id)}
                          disabled={busyId === p.id}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 hover:bg-orange-100 text-orange-700 rounded-full text-xs font-medium transition-colors disabled:opacity-50"
                        >
                          <Tag className="w-3 h-3" />
                          {busyId === p.id ? '...' : "Aksiyaga qo'shish"}
                        </button>
                      </td>
                    </tr>
                  ))}
